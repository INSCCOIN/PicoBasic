...
CLS
PRINT "PicoSprite 1.2"
PRINT "Arrow keys: Move | Space: Toggle | S: Save | L: Load | Q: Quit"
PRINT "F/G: Prev/Next Frame | A: Animate | H: Help/About"
PRINT
...
DO
    ...
    IF UCASE$(key$) = "H" THEN
        CLS
        PRINT "PicoSprite 1.2 - Help & About"
        PRINT STRING$(32, "-")
        PRINT "Arrow keys: Move cursor"
        PRINT "Space:      Toggle pixel on/off"
        PRINT "F/G:        Previous/Next frame (max 8 frames)"
        PRINT "A:          Animate all frames"
        PRINT "S:          Save all frames to sprite.txt"
        PRINT "L:          Load all frames from sprite.txt"
        PRINT "Q:          Quit the editor"
        PRINT
        PRINT "Each frame is 8x8 pixels. Use multiple frames"
        PRINT "to create simple sprite animations."
        PRINT
        PRINT "Author: INSCCOIN"
        PRINT "PicoSprite Version 1.2"
        PRINT "Framework Version: 1.6" 
        PRINT "MMBasic Version: (6.00.02 RC26)"
        PRINT "Year: 2025"
        PRINT
